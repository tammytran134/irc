                 ...
                 connection_info_t *connection, client_info_t *client)
{
    ...
    {
        server_reply(":You have not registered", 
                        reply_code, connection, client);
    }
    ...
    {
        server_reply(":Unauthorized command (already registered)", 
                            reply_code, connection, client);
    }
    else if (strcmp(reply_code, ERR_PASSWDMISMATCH) == 0)
    {
        server_reply(":Password incorrect", 
                        reply_code, connection, client);
    }
...
void server_reply(char *msg, char *reply_code, 
        connection_info_t *connection, client_info_t *client)
{
    char reply_msg[MAX_LEN_STR];
    ...
    sprintf(reply_msg, ":%s %s %s %s\r\n", connection->server_hostname, 
                                            reply_code, client_nickname, msg);
